...
{
	$result = $spotify->webRequestApi('/v1/me/player/currently-playing', $_GET['token']);
	if($result['success'])
	{
		$items = array();
		$item = $result['item'];
		$artists = array();
		foreach($item['artists'] as $artist) $artists[] = $artist['name'];
		$items[] = array(
			'name' => $item['name'],
...
$authURL = $spotify->getUserAuthHref('user-read-currently-playing');
